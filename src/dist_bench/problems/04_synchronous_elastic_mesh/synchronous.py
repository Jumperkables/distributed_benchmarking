...
def main():
    ...
    while time.time()-start < 0.001:
        rprint(f"{time.time()-start:.6f} | Avg of tensor {tensor.mean().item():.6f} | Completed?: {big_reduce_handle.is_completed()}")
    big_reduce_handle.wait()
    rprint(f"After wait: avg of tensor {tensor.mean().item():.6f} |")
